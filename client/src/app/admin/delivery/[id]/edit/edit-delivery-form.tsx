...
import { UpdateDeliveryBodySchema } from "@/validation-schema/admin/delivery";
...
interface EditDeliveryFormProps {
  delivery: any; // Using any for now since we don't have the exact type
}

export default function EditDeliveryForm({ delivery }: EditDeliveryFormProps) {
  ...
  const {
    ...
    resolver: zodResolver(UpdateDeliveryBodySchema),
    defaultValues: {
      ...
      address: delivery.recipientAddress?.address || "",
      province:
        delivery.recipientAddress?.province || locations[0]?.label || "",
      district:
        delivery.recipientAddress?.district ||
        locations[0]?.districts[0]?.label ||
        "",
      ward: delivery.recipientAddress?.ward || "",
      deliveryDate: new Date(delivery.deliveryDate) || new Date(),
      deliveryPeriod: delivery.deliveryPeriod || periodOptions[0]?.value || "",
  ...
  const onSubmit = async (data: FormData) => {
    try {
      ...
      await AdminDeliveryRequestApis.update(delivery.id, deliveryData);

      toast({
        title: "Thành công",
        description: "Đã cập nhật giao hàng thành công",
      });

      router.push(routePath.admin.delivery.detail(delivery.id));
    } catch (error) {
      toast({
        title: "Lỗi",
        description:
          (error as Error).message || "Có lỗi xảy ra khi cập nhật giao hàng",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4">
          <Button variant="outline" asChild>
            <Link href={routePath.admin.delivery.detail(delivery.id)}>Hủy</Link>
          </Button>
          <Button type="submit" disabled={isLoading}>
            {isLoading ? "Đang cập nhật..." : "Cập nhật giao hàng"}
          </Button>
        </div>
      </form>
    </div>
